...
class mainApp(QMainWindow, transcodeWinUI.Ui_MainWindow):
    ...
    def onFinished(self, exitCode, exitStatus):
        ...
        if exitCode == 0:
            ...
            self.log.log(
                f"Dauer: {time_str}; ReturnCode={exitCode} (ExitStatus={exitStatus})"
            )
            self.log.log("OK")
            try:
    ...
    def getCmd(self):
        ...
        if self.video.typ == "SD":
            self.cmd = config.get(
                "SD",
                "cmd",
                fallback='ffmpeg -hide_banner {canvassize} -hwaccel auto -i "{EingabeDatei}"  -map 0 -c:v hevc_nvenc -preset fast -profile:v main10 -pix_fmt p010le -crf 28 -b:v 0 -maxrate 2M -bufsize 4M -dn -c:a copy -c:s dvdsub -y "{AusgabeDatei}"',
            )
        elif self.video.typ == "HD":
            self.cmd = config.get(
                "HD",
                "cmd",
                fallback='ffmpeg -hide_banner {canvassize} -hwaccel auto -i "{EingabeDatei}"  -map 0 -c:v hevc_nvenc -preset slow -profile:v main10 -pix_fmt p010le -crf 23 -b:v 0 -maxrate 3M -bufsize 6M -dn -c:a copy -c:s dvdsub -y "{AusgabeDatei}"',
            )
        else:
            self.cmd = config.get(
                "FullHD",
                "cmd",
                fallback='ffmpeg -hide_banner {canvassize} -hwaccel auto -i "{EingabeDatei}"  -map 0 -c:v hevc_nvenc -preset slow -profile:v main10 -pix_fmt p010le -crf 23 -b:v 0 -maxrate 4M -bufsize 8M -dn -c:a copy -c:s dvdsub -y "{AusgabeDatei}"',
            )
    ...
    def openFileNameDialog(self) -> str:
        ...
        fileName, _ = QFileDialog.getOpenFileName(
            ...
            "E:\\Filme\\",
            "All Files (*);;Filme (*.mkv *.mpg *.mp4 *.ts *.avi)",
            options=options,
        )
        if fileName:
